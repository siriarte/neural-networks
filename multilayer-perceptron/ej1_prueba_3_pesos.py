...
def variacion_pesos():
    ...
    red_0 = rn.RedNeuronal(cantidad_neuronas_entrada, [10, 10], cantidad_neurona_salida, 0, 1, 0, 0, 0, '', 0)
    ...
    # Entreno red - Momentum = 0.1
    red_1 = rn.RedNeuronal(cantidad_neuronas_entrada, [10, 10], cantidad_neurona_salida, 0, 1, 0, 0, 0, '', 1)
    ...
    plt.cla()
    plt.clf()
    plt.plot(x0, y0, label='Distribucion Normal')
    plt.plot(x1, y1, label='Distribucion Uniforme')
    plt.xlabel('EPOCAS')
    plt.ylabel('ERROR ENTRENAMIENTO')
    plt.grid(True)
    plt.legend(loc='upper right', frameon=False)
    plt.savefig("graficos\\ej1_prueba_3_pesos_entrenamiento.png")
    #plt.show()
    ...
    plt.grid(True)
    plt.legend(loc='upper right', frameon=False)
    plt.savefig("graficos\\ej1_prueba_3_pesos_validacion.png")
    #plt.show()


    print('Distribucion Normal ---> TEST = %f' % (red_0.test_ej1(datos_testing_entrada, datos_testing_resultados)))
    print('Distribucion Uniforme ---> TEST = %f' % (red_1.test_ej1(datos_testing_entrada, datos_testing_resultados)))
