...
def variacion_early_stopping():
    ...
    # Entreno red - Momentum = 0.1
    red_1 = rn.RedNeuronal(cantidad_neuronas_entrada, [5, 5], cantidad_neurona_salida, 0, 1, 0.001, 0, 0, '')
    ...
    red_2 = rn.RedNeuronal(cantidad_neuronas_entrada, [5, 5], cantidad_neurona_salida, 0, 1, 0.005, 0, 0, '')
    ...
    red_3 = rn.RedNeuronal(cantidad_neuronas_entrada, [5, 5], cantidad_neurona_salida, 0, 1, 0.009, 0, 0, '')
    ...
    plt.plot(x1, y1, label='EarlyStopping = 0.001')
    plt.plot(x2, y2, label='EarlyStopping = 0.005')
    plt.plot(x3, y3, label='EarlyStopping = 0.009')
    plt.xlabel('EPOCAS')
    plt.ylabel('ERROR ENTRENAMIENTO')
    plt.grid(True)
    plt.legend(loc='upper right', frameon=False)
    plt.savefig("graficos\\ej2_prueba_5_earlystopping_momentum_entrenamiento.png")
    #plt.show()
    ...
    plt.grid(True)
    plt.legend(loc='upper right', frameon=False)
    plt.savefig("graficos\\ej2_prueba_5_earlystopping_momentum_validacion.png")
    # plt.show()

    print('EarlyStopping = 0---> TEST = %f' % (red_0.test_ej2(datos_testing_entrada, datos_testing_resultados)))
    print('EarlyStopping = 0.001 ---> TEST = %f' % (red_1.test_ej2(datos_testing_entrada, datos_testing_resultados)))
    print('EarlyStopping = 0.005 ---> TEST = %f' % (red_2.test_ej2(datos_testing_entrada, datos_testing_resultados)))
    print('EarlyStopping = 0.009 ---> TEST = %f' % (red_3.test_ej2(datos_testing_entrada, datos_testing_resultados)))
